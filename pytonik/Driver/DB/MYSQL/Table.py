...
class Table:
    ...
    def whereColumn(self, *variable):
        ...
        if (type(variables) == tuple) == True:
            ...
            for v in variable:
                lv.append(" ".join(v))
            self.table_select = "SELECT FROM {table} WHERE {variable} ".format(table=self.table, variable=" AND ".join(lv))

        elif (type(variables) == str) == True:

            self.table_select = "SELECT FROM {table} WHERE {variable} {sign} {string} ".format(table=self.table, variable=variables, sign=sign, string=string)
    ...
    def whereNotBetween(self, column, values=[]):
        ...
        self.table_whereNotBetween = " {column} NOT BETWEEN {values} ".format(column=column, values=" AND ".join(values))
    ...
    def outerJoin(self, *variable):
        ...
        table_outerjoin = " LEFT OUTER JOIN  {table} ON {variable} {sign} {string}".format(table=str(table), variable = variables, sign=sign, string=string)
    ...
    def leftJoin(self, *variable):
        ...
        table_leftjoin = " LEFT JOIN {table} ON {variable} {sign} {string}".format(table=str(self.prefix)+str(table), variable = variable, sign=sign, string=string)
    ...
    def rightJoin(self, *variable):
        ...
        table_rightjoin = " RIGHT JOIN {table} ON {variable} {sign} {string}".format(table=str(self.prefix)+str(table), variable = variable, sign=sign, string=string)
    ...
    def update(self, data=[]):
        if (type(data) == list):
            if len(data) > 0:
                ...
                t_result = self.query(table_update)
                return t_result.save() if t_result.Exception == "" else t_result.Exception
            else:
                return "Empty Data"
        else:
    ...
    def create(self):
        if type(self.tabledict) == dict:
            for table_name in self.tabledict:
                ...
                t_result = self.query(table_description)
            return t_result.save() if t_result.Exception == "" else t_result.Exception
        else:
