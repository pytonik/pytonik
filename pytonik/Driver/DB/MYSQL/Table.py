...
class Table:
    ...
    def drop(self):
        self.table_drop = "DROP TABLE {exists} {table}".format(
            table=self.table,
            exists="IF " + str(self.table_exist) if self.table_exist is not "" else "",
        )
    ...
    def exists(self, rawquery=""):
        rawString = (
            "WHERE EXISTS ({rawquery})".format(rawquery=rawquery)
            if rawquery is not ""
            else "EXISTS"
        )
    ...
    def notExist(self, rawquery=""):
        rawString = (
            "WHERE NOT EXISTS ({rawquery})".format(rawquery=rawquery)
            if rawquery is not ""
            else "NOT EXISTS"
        )
    ...
    def select(self, *value):
        ...
        self.table_select = "SELECT {value}{distinct}{values}{max}{min}{count}{avg} FROM {table}{exists}{notexist}{outerjoin}{join}{leftjoin}{rightjoin}{where}{whereisnull}{wherenotnull}{whereBetween}{whereNotBetween}{wherenotin}{orwhere}{groupBy}{having}{orderBy}{limit}{take}".format(
            ...
            whereBetween=str(" WHERE") + self.table_whereBetween
            if len(self.table_whereBetween) > 0
            else "",
            whereNotBetween=str(" WHERE") + self.table_whereNotBetween
            if len(self.table_whereNotBetween) > 0
            else "",
            ...
            whereisnull=str(" WHERE") + str("".join(self.table_whereisnull))
            if len(self.table_whereisnull) > 0
            else "",
            wherenotnull=str(" WHERE") + str("".join(self.table_wherenotnull))
            if len(self.table_wherenotnull) > 0
            else "",
            where=str(" WHERE") + str(_and.join(self.table_where))
            if len(self.table_where) > 0
            else "",
            wherenotin=str(_where) + str(_and) + str(self.table_wherenotin)
            if len(self.table_wherenotin) > 0 is not ""
            else "",
    ...
    def count(self, *column):
        ...
        self.table_count = "COUNT({column}) {sign} {string} ".format(
            column=variables, sign=sign, string=columnas
        )
        return self
    ...
    def whereAnd(self, variable="", sign="", string=""):
        string_type = (
            ...
            if (type(string) == str) == True
            else string
        )
        self.table_distinct = "AND {variable} {sign} {string}".format(
            variable=variable, sign=sign, string=string_type
        )
        return self

    def whereIn(self, variable, list=[]):

        self.table_wherenotin = "{variable} NOT IN {list}".format(
            variable=variable, list="({})".format(",".join(list))
        )
    ...
    def orOn(self, variable):
        if len(variable) > 2:
            ...
            sign = "="
            string = variable[1]
        table_orOn = " OR {variable} {sign} {string}".format(
    ...
    def on(self, variable):
        ...
        self.table_On = " ON {variable} {sign} {string}".format(
            variable=variables, sign=sign, string=string
        )
    ...
    def orWhere(self, *variable):
        ...
        table_orwhere = "{variable} {sign} {string}".format(
            variable=variables, sign=sign, string=string_type
        )
    ...
    def whereColumn(self, *variable):
        ...
        if (type(variables) == tuple) == True:
            ...
            for v in variable:
                lv.append(" ".join(v))
            self.table_select = "SELECT FROM {table} WHERE {variable} ".format(
                table=self.table, variable=" AND ".join(lv)
            )

        elif (type(variables) == str) == True:

            self.table_select = "SELECT FROM {table} WHERE {variable} {sign} {string} ".format(
                table=self.table, variable=variables, sign=sign, string=string
            )
    ...
    def whereBetween(self, column, values=[]):
        ...
        self.table_whereBetween = " {column} BETWEEN {values} ".format(
            column=column, values=" AND ".join(values)
        )
    ...
    def whereNotBetween(self, column, values=[]):
        ...
        self.table_whereNotBetween = " {column} NOT BETWEEN {values} ".format(
            column=column, values=" AND ".join(values)
        )
    ...
    def whereBetweenRaw(self, column, values=[]):
        ...
        column = " {column} BETWEEN {values} ".format(
            column=column, values=" AND ".join(values)
        )
    ...
    def where(self, *variable):
        ...
        table_where = " {variable} {sign} {string}".format(
            variable=variables, sign=sign, string=string_type
        )
    ...
    def delete(self):
        self.table_delete = "DELETE FROM {table} {where}".format(
            table=self.table, where=self.table_where
        )
    ...
    def limit(self, limits):
        ...
        self.table_limit = " LIMIT {offset} {limit}".format(
            offset=self.table_offset, limit=limits
        )
        return self
    ...
    def take(self, limits):
        ...
        self.table_take = " LIMIT {offset} {limit}".format(
            offset=self.table_offset, limit=limits
        )
        return self
    ...
    def join(self, *variable):
        ...
        table_join = " JOIN {table} ON {variable} {sign} {string}".format(
            table=str(self.prefix) + str(table),
            variable=variables,
            sign=sign,
            string=string,
        )
    ...
    def outerJoin(self, *variable):
        ...
        table_outerjoin = " LEFT OUTER JOIN  {table} ON {variable} {sign} {string}".format(
            table=table, variable=variables, sign=sign, string=string
        )
    ...
    def leftJoin(self, *variable):
        ...
        table_leftjoin = " LEFT JOIN {table} ON {variable} {sign} {string}".format(
            table=str(self.prefix) + str(table),
            variable=variables,
            sign=sign,
            string=string,
    ...
    def pluck(self, column, key=""):
        ...
        self.table_select = "SELECT {column} FROM {table} ".format(
            table=str(self.table),
            column="*" if len(column) < 1 else " {}.{}".format(str(self.table), column),
        )
        self.get()
        kl = []
        if key is "":
                ...
                for ks, vs in v.items():
                    ...
                    if num > 0:
                        ...
                        if "{" + lf + "}" in funcquery:
                            ...
                            table_update = str(funcquery).replace(
                                "{" + lf + "}", str(vs)
                            )
        ...
        if type(data) == list:
            if len(data) > 0:
                ...
                table_update = "UPDATE {table} SET {column} {where}".format(
                    table=str(self.table),
                    column=lcolumn,
                    where=str(" WHERE") + str("AND".join(self.table_where))
                    if len(self.table_where) > 0
                    else "",
                )
                t_result = self.DB.query(table_update)
                return (
                    t_result.save() if t_result.Exception == "" else t_result.Exception
                )
            else:
                return "Empty Data"
        else:
